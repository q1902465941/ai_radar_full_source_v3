...
def test_learning_data_audit_requires_real_closed_samples_even_with_passing_market_backtest(monkeypatch):
    learning_data_audit.clear_cache()
    monkeypatch.setattr(settings, "replay_enabled", True)
    monkeypatch.setattr(replay_memory, "samples", lambda limit=None: [sample_trade(close_time=idx + 1) for idx in range(60)])
    ...
    monkeypatch.setattr(
        db,
        "list_closed",
        lambda limit=100000: [{"close_reason": "TP2"} for _ in range(7)],
    )
    monkeypatch.setattr(db, "get_kv", lambda key, default=None: default)
    monkeypatch.setattr(
        learning_data_audit,
        "_radar_summary",
        lambda: {"ok": True, "span_days": 14.0, "rows": 1000, "distinct_scans": 300, "distinct_symbols": 30},
    )
    monkeypatch.setattr(
        learning_data_audit,
        "_market_backtest_summary",
    ...
    assert "replay_dominated" in report["reasons"]
...
def test_paper_graduation_progress_does_not_treat_backtest_as_real_sample_substitute():
    import backend.system_readiness as system_readiness

    progress = system_readiness.paper_graduation_progress(
        {
            "production_grade": False,
            "trust_level": "MEDIUM",
            ...
            "sources": {
                "combined_samples": 67,
                "replay_samples": 60,
                "real_closed_samples_with_radar": 7,
                "codex_real_closed_samples_with_radar": 2,
                "real_closed_samples_by_provider": {"codex_cli": 2, "rule": 5},
                "replay_ratio": 0.8955,
            },
            "radar_snapshots": {"span_days": 14.0},
            "market_backtest": {"available": True, "quality_passed": True},
        }
    )

    assert progress["market_backtest_quality_passed"] is True
    assert progress["missing_real_closed_samples"] == 23
    assert progress["next_requirement"] == (
        "Collect 23 more real closed paper/shadow samples with radar context; "
...
def test_production_acceptance_prioritizes_geometry_supported_strict_candidates(monkeypatch):
    weak = high_quality_item(symbol="GEOMETRYWEAKSTRICTUSDT", side="LONG", price=100)
    strong = high_quality_item(symbol="GEOMETRYSTRONGSTRICTUSDT", side="LONG", price=100)
    weak.score = 100
    strong.score = 82
    radar_engine.top50 = [weak, strong]
    weak_sample = {
        "status": "weak",
        "sample_model": "first_touch_geometry_v1",
        "symbol": weak.symbol,
        "side": weak.direction,
        "pass_count": 0,
        "samples": {"sample_count": 119, "win_rate": 0.42, "expected_r": 0.04, "profit_factor": 0.9, "pass_gate": False},
    }
    strong_sample = ok_strategy_geometry_sample(strong)
    geometry_reports = [
        {"symbol": weak.symbol, "geometry_status": "weak", "sample_count": 119, "win_rate": 0.42, "expected_r": 0.04, "profit_factor": 0.9, "pass_count": 0},
        {"symbol": strong.symbol, "geometry_status": "ok", "sample_count": 120, "win_rate": 0.64, "expected_r": 0.42, "profit_factor": 1.82, "pass_count": 12},
    ]
    seen = []

    async def fake_scan(force_refresh=False):
        radar_engine.top50 = [weak, strong]
        radar_engine.last_scan_id = "scan_geometry_rank_test"
        radar_engine.last_scan_time = "12:00:00"
        return radar_engine.top50

    async def fake_geometry_order(candidates, candidate_source, performance_context=None):
        assert [item.symbol for item in candidates] == [weak.symbol, strong.symbol]
        ...
        return [strong, weak], geometry_reports

    async def fail_geometry(candidate):
        raise AssertionError("production acceptance should reuse pre-ranked geometry samples")

    async def fake_generate(candidate, performance, candidate_source, candidate_attempts, strategy_geometry_sample=None):
        seen.append((candidate.symbol, strategy_geometry_sample))
        return StrategyPlan(
            strategy_id=f"wait_{candidate.symbol}",
            action="WAIT",
            symbol=candidate.symbol,
            side="NEUTRAL",
            entry_zone_low=candidate.price,
    ...
    monkeypatch.setattr(learning_data_audit, "summary", lambda force=False, limit=None: {"production_grade": True, "trust_level": "PRODUCTION", "reasons": []})
    monkeypatch.setattr(radar_engine, "scan", fake_scan)
    monkeypatch.setattr(radar_engine, "select_ai_candidates", lambda items: [weak, strong])
    monkeypatch.setattr(autotrader, "_candidate_batch", lambda performance: ([weak, strong], "strict"))
    monkeypatch.setattr(autotrader, "_geometry_supported_candidate_order", fake_geometry_order)
    monkeypatch.setattr("backend.trading.production_acceptance.strategy_geometry_sampler.evaluate", fail_geometry)
    monkeypatch.setattr(production_acceptance_runner, "_generate_plan", fake_generate)
    monkeypatch.setattr(
        autotrader,
        "_ai_candidate_freshness_report",
...
def test_production_acceptance_uses_strict_geometry_candidate_when_only_cyqnt_blocks(monkeypatch):
    item = high_quality_item(symbol="STRICTGEOMETRYUSDT", side="LONG", price=100)
    ok_sample = ok_strategy_geometry_sample(item)
    seen = []

    async def fake_scan(force_refresh=False):
        radar_engine.top50 = [item]
        radar_engine.last_scan_id = "scan_strict_geometry_test"
        radar_engine.last_scan_time = "12:00:00"
        return radar_engine.top50
    ...
    async def fake_geometry_order(candidates, candidate_source, performance_context=None):
        assert [candidate.symbol for candidate in candidates] == [item.symbol]
        assert candidate_source == "strict_geometry"
        autotrader._candidate_geometry_samples[item.symbol] = ok_sample
        return [item], [
            {
    ...
    async def fake_generate(candidate, performance, candidate_source, candidate_attempts, strategy_geometry_sample=None):
        seen.append((candidate.symbol, candidate_source, strategy_geometry_sample))
        return StrategyPlan(
            strategy_id="strict_geometry_wait",
            action="WAIT",
            symbol=candidate.symbol,
            side="NEUTRAL",
            entry_zone_low=candidate.price,
    ...
    assert evidence["candidate_source"] == "strict_geometry"
    assert evidence["candidate_symbols"] == [item.symbol]
    assert evidence["candidate_attempts"][0]["strict_geometry_symbols"] == [item.symbol]
    assert evidence["candidate_attempts"][0]["strict_geometry_candidate_reports"][0]["geometry_status"] == "ok"
    assert seen == [(item.symbol, "strict_geometry", ok_sample)]
...
def test_production_acceptance_status_invalidates_pass_when_current_learning_data_is_low(monkeypatch):
    ...
    def fake_cached_audit_summary():
        cached_audit_calls.append(True)
        return {
            "production_grade": False,
            "trust_level": "LOW",
            "reasons": ["real_closed_samples_low", "market_backtest_missing"],
    ...
    assert out["production_acceptance"]["currently_valid"] is False
    assert "learning_data_not_production_grade" in out["current_validation"]["invalidated_by"]
    assert out["current_validation"]["learning_data_audit"]["trust_level"] == "LOW"
    assert cached_audit_calls == [True]


def test_production_acceptance_status_does_not_recompute_uncached_learning_audit(monkeypatch):
    production_acceptance_runner.last_report = {
        "ok": True,
        "mode": "real_order",
        "finished_ms": now_ms(),
    ...
    def fail_summary(force=False, limit=5000):
        raise AssertionError("status endpoint must not recompute full data audit")

    monkeypatch.setattr(learning_data_audit, "summary", fail_summary)
    ...
    assert out["production_acceptance"]["passed"] is False
    assert "learning_data_not_production_grade" in out["current_validation"]["invalidated_by"]
    assert out["current_validation"]["learning_data_audit"]["trust_level"] == "UNKNOWN"
...
def test_production_acceptance_skips_codex_when_strategy_geometry_is_weak(monkeypatch):
    item = high_quality_item(symbol="GEOMETRYBLOCKUSDT", side="LONG", price=100)
    weak_sample = {
        "status": "weak",
        "sample_model": "first_touch_geometry_v1",
        "symbol": item.symbol,
        "side": "LONG",
        "pass_count": 0,
        "samples": {"sample_count": 119, "win_rate": 0.46, "expected_r": 0.05, "profit_factor": 1.03, "pass_gate": False},
    }
    ...
    async def fail_generate(*args, **kwargs):
        raise AssertionError("weak geometry should block before Codex is called")
    ...
    assert out["provider"] == "strategy_geometry_gate"
    assert out["plan"].action == "WAIT"
    assert out["plan_attempts"][0]["provider"] == "strategy_geometry_gate"
    assert out["plan_attempts"][0]["validation_reason"] == "strategy_geometry_sample_not_ok"
    assert out["plan_attempts"][0]["strategy_geometry_sample"] == weak_sample


def test_production_acceptance_passes_valid_strategy_geometry_to_codex(monkeypatch):
    item = high_quality_item(symbol="GEOMETRYOKUSDT", side="SHORT", price=100)
    ok_sample = {
        "status": "ok",
        "sample_model": "first_touch_geometry_v1",
        "symbol": item.symbol,
        ...
        "pass_count": 7,
        "selected_geometry": {"side": "SHORT", "entry": 100, "stop_loss": 101, "tp1": 99, "tp2": 97},
        "samples": {"sample_count": 119, "win_rate": 0.64, "expected_r": 0.42, "profit_factor": 1.82, "pass_gate": True},
    }
    seen_geometry = []
    ...
    async def fake_generate(candidate, performance, candidate_source, candidate_attempts, strategy_geometry_sample=None):
        seen_geometry.append(strategy_geometry_sample)
        return StrategyPlan(
            strategy_id="geometry_ok_wait",
            action="WAIT",
            symbol=candidate.symbol,
            side="NEUTRAL",
            entry_zone_low=candidate.price,
...
def test_production_acceptance_generate_plan_overrides_gate_for_strict_geometry(monkeypatch):
    item = high_quality_item(symbol="STRICTGEOMETRYCTXUSDT", side="SHORT", price=100)
    ok_sample = ok_strategy_geometry_sample(item)
    original_feedback = {
        "candidate_feedback": {
            "quality_bias": "NEUTRAL",
            "generation_gate": {
                ...
                "review_required": False,
                "reasons": ["cyqnt_estimated_win_rate_low"],
                "instruction": "test original gate",
            },
        }
    }
    seen_contexts = []
    ...
    async def fake_generate(candidate, position_context=None):
        seen_contexts.append(position_context or {})
        compressed = context_compressor.build_strategy_context(candidate, position_context)
        gate = compressed["ai_strategy_quality_feedback"]["candidate_feedback"]["generation_gate"]
        assert gate["allow_open_plan"] is True
        assert gate["reasons"] == []
        assert gate["override_reason"] == "strict_geometry_supported"
        assert gate["overridden_generation_gate"]["reasons"] == ["cyqnt_estimated_win_rate_low"]
        return StrategyPlan(
            strategy_id="strict_geometry_context_wait",
            action="WAIT",
            symbol=candidate.symbol,
            side="NEUTRAL",
            entry_zone_low=candidate.price,
    ...
    monkeypatch.setattr(ai_strategy_feedback, "compact_context", lambda candidate: original_feedback)
    monkeypatch.setattr("backend.trading.production_acceptance.ai_service.generate_strategy", fake_generate)
    ...
    plan = asyncio.run(
        production_acceptance_runner._generate_plan(
            ...
            {"recovery_mode": False},
            "strict_geometry",
            [{"attempt": 1, "strict_geometry_symbols": [item.symbol]}],
            strategy_geometry_sample=ok_sample,
        )
    )

    selection = seen_contexts[0]["candidate_selection"]
    gate = seen_contexts[0]["ai_strategy_quality_feedback"]["candidate_feedback"]["generation_gate"]
    assert plan.raw["provider"] == "codex_cli"
    assert selection["source"] == "production_acceptance"
    assert selection["candidate_source"] == "strict_geometry"
    assert selection["acceptance_mode"] is True
    assert selection["strict_geometry_candidate"] is True
    assert gate["allow_open_plan"] is True
    assert gate["override_reason"] == "strict_geometry_supported"
...
def ok_strategy_geometry_sample(item):
    ...
    price = float(item.price or 100.0)
    risk = price * 0.01
    return {
        "status": "ok",
        "sample_model": "first_touch_geometry_v1",
        ...
        "selected_geometry": {
            "side": item.direction,
            "entry": price,
            "stop_loss": price - risk if is_long else price + risk,
            "tp1": price + risk * 1.2 if is_long else price - risk * 1.2,
            "tp2": price + risk * 3.0 if is_long else price - risk * 3.0,
            "tp2_r": 3.0,
        },
        "samples": {
            "sample_count": 120,
...
def patch_ok_production_strategy_geometry(monkeypatch):
    async def fake_geometry(item):
        return ok_strategy_geometry_sample(item)
